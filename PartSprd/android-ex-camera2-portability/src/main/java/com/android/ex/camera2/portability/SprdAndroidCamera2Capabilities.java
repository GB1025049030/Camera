...
import android.hardware.camera2.help.CameraCharacteristicsHelp;
...
public class SprdAndroidCamera2Capabilities extends CameraCapabilities {
    ...
    private void buildFaceAttributesEnable(CameraCharacteristics p){
        try {
            if (!CameraCharacteristicsHelp.hasField("CONTROL_AVAILABLE_FACEFEATUREENABLE")) {
                return;
            }
    ...
    private void buildZoomRatioSection(CameraCharacteristics p){
        float[] sprdZoomRatioSection =
                CameraCharacteristicsHelp.hasField("ANDROID_SPRD_ZOOM_RATIO_SECTION")
                        ? p.get(ANDROID_SPRD_ZOOM_RATIO_SECTION) : null;
    ...
    private void buildLogoWatermarkEnable(CameraCharacteristics p) {
        int supportedLogoWatermark =
                CameraCharacteristicsHelp.hasField("CONTROL_AVAILABLE_LOGOWATERMARK")
                        ? p.get(CONTROL_AVAILABLE_LOGOWATERMARK) : 0;
    ...
    private void buildTimeWatermarkEnable(CameraCharacteristics p) {
        int supportedTimeWatermark =
                CameraCharacteristicsHelp.hasField("CONTROL_AVAILABLE_TIMEWATERMARK")
                        ? p.get(CONTROL_AVAILABLE_TIMEWATERMARK) : 0;
