...
import com.binbin.camera.activity.GLFilterActivity;
 ...
 * </p>
 */
public abstract class QuickActivity extends GLFilterActivity<TextureView, byte[]> {
    ...
    private int currentBrightnessMode = -1;
    private int currentBrightnessValue = -1;
    ...
    protected final void onResume() {
        ...
        if (!TextUtils.isEmpty(CameraCustomManager.I.getCameraCustomBrightness())) {
            int cameraCustomBrightness = Integer.parseInt(CameraCustomManager.I.getCameraCustomBrightness());
            currentBrightnessMode = FreemeCameraUtil.getBrightnessMode(this);
            currentBrightnessValue = FreemeCameraUtil.getBrightnessValue(this);
            if (currentBrightnessMode != 0) {
                FreemeCameraUtil.setBrightnessMode(this, 0);
            }
            FreemeCameraUtil.setBrightnessValue(this, cameraCustomBrightness);
        }
    ...
    protected final void onPause() {
        ...
        if (currentBrightnessMode != -1){
            FreemeCameraUtil.setBrightnessMode(this, currentBrightnessMode);
        }
        if (currentBrightnessValue != -1) {
            FreemeCameraUtil.setBrightnessValue(this, currentBrightnessValue);
        }
        mMainHandler.removeCallbacks(mOnResumeTasks);
