...
public class FocusOverlayManager implements PreviewStatusListener.PreviewAreaChangedListener,
    ...
    public void updateCapabilities(CameraCapabilities capabilities) {
        ...
        mFocusAreaSupported = mCapabilities.supports(CameraCapabilities.Feature.FOCUS_AREA) && (mIsBackCamera || FreemeCameraUtil.isFrontFocusAreaEnable());
    ...
    public void setCameraId(int cameraId) {
        mIsBackCamera = DreamUtil.getRightCamera(cameraId) == DreamUtil.BACK_CAMERA;
    }
    ...
    public void onSingleTapUp(int x, int y) {
        ...
        if (mAutoChasingSupported && mAutoChasingEnable){
            ...
            Log.d(TAG,"set chasing Region x = " + x +" , y = " + y);
            initializeAutoChasingRegion(x,y);
            mListener.setAutoChasingParameters();
            return;
        }
    ...
    private Rect computeCameraRectFromPreviewCoordinates(int x, int y, int size) {
        ...
        int top = CameraUtil.clamp(y - size / 2, 0,
                mPreviewRect.bottom - mPreviewRect.top - size);
