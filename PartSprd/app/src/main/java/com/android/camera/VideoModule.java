...
import static com.android.camera.settings.Keys.KEY_CAMERA_SHUTTER_SOUND;
...
public class VideoModule extends CameraModule implements
    ...
    private static final String CAMERA_SHUTTER_SOUND_ON_VALUE = "1";
    ...
    private static final String PRODUCT_VIDEO_RECORD = "/product/media/audio/ui/VideoRecord.ogg";
    private static final String SYSTEM_VIDEO_RECORD = "/system/media/audio/ui/VideoRecord.ogg";
    private static final long PERIOD = 100;
    private int mVideoRecordAudioLength;
    ...
    public void init(CameraActivity activity, boolean isSecureCamera,
        ...
        mVideoRecordAudioLength = getAudioLength(PRODUCT_VIDEO_RECORD, SYSTEM_VIDEO_RECORD);
    }
        ...
        if (!isCameraFrontFacing() && CameraUtil.isEOISDvBackEnabled()) {
            if (mDataModuleCurrent.isEnableSettingConfig(Keys.KEY_EOIS_DV_BACK) && mDataModuleCurrent.getBoolean(Keys.KEY_EOIS_DV_BACK)) {
                ...
                try {
                    ...
                    setVideoStabilization.invoke(mMediaRecorder);
                } catch (Exception e) {
                    e.printStackTrace();
...
//                }
    ...
    private void onMediaRecodeStart() {
        ...
        Observable<Long> observable = Observable.interval(0, PERIOD, TimeUnit.MILLISECONDS)
                .takeWhile(aLong -> aLong * PERIOD <= mVideoRecordAudioLength)
                .subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread());
        Disposable disposable = observable.subscribeWith(new DisposableObserver<Long>() {
            @Override
            ...
            public void onComplete() {
                Log.i(TAG, "onVideoStart , mPaused = " + mPaused); // add log for Bug 915151. callback be set to null after null-judgement
                if (mActivity != null && !mPaused && mCameraDevice != null && mMediaRecorder != null) {
                    mActivity.runOnUiThread(mMediaRecoderStartRunnable);
                }
            }
        });
        DisposableManager.I.add(disposable);
    }

    private String getAudioContentUri(Context context, String audioFilePath) {
        ...
        if (audioFile.exists()) {
            ...
            if (cursor != null && cursor.moveToFirst()) {
                ...
                Uri baseUri = Uri.parse("content://media/internal/audio/media");
    ...
    private int getAudioLength(String... pathArray) {
        int duration = 0;
    ...
    private int getAudioLength(String path) {
        String uriStr = getAudioContentUri(mActivity, path);
        int duration = -1;
...
//            startPreview(false);
    ...
    public void onDreamSettingChangeListener(HashMap<String, String> keys) {
        ...
        for (String key : keys.keySet()) {
            ...
            if (KEY_CAMERA_SHUTTER_SOUND.equals(key)) {
                if (CAMERA_SHUTTER_SOUND_ON_VALUE.equals(keys.get(key))) {
...
//    }
    ...
    public boolean isMakeUpEnable() {
        /*/ C:20201102 Modify set makeupEnable to false
