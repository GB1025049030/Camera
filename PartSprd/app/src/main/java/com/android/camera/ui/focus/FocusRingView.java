 ...
 */
public class FocusRingView extends View implements Invalidator, FocusRing, PreviewStatusListener.PreviewAreaChangedListener {
    ...
    private RectF mRealPreviewSize = new RectF();
    ...
    public void configurePreviewDimensions(RectF previewArea) {
        ...
        mRealPreviewSize.set(previewArea.left, 0, previewArea.right, previewArea.bottom - previewArea.top);
...
//        mAnimator.draw(canvas);
        ...
        if (mRealPreviewSize != null) {
            canvas.save();
            canvas.clipRect(mRealPreviewSize);
            mAnimator.draw(canvas);
            canvas.restore();
        } else {
            mAnimator.draw(canvas);
    ...
    private Point computeCenter() {
        if (mPreviewSize != null && (mPreviewSize.width() * mPreviewSize.height() > 0.01f)) {
            ...
            return new Point((int) mPreviewSize.centerX(), (int) mPreviewSize.centerY() - (int) mPreviewSize.top);
        }
