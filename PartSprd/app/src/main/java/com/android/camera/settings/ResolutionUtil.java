...
import com.freeme.camera.common.custom.CameraCustomInterpol;
...
public class ResolutionUtil {
    ...
    private static double degressiveRatio = 0.5;
    ...
    public static List<Size> getDisplayableSizesFromSupported(List<Size> sizes, boolean isBackCamera) {
        if (FreemeCameraUtil.isPictureSizeCustomEnable()) {
            CameraCustomInterpol.filterSize(sizes, isBackCamera);
        }
        ...
        if (!(TextUtils.isEmpty(CameraCustomManager.I.getPictureSizeSelectorDegressiveRatio()))) {
            degressiveRatio = Double.parseDouble(CameraCustomManager.I.getPictureSizeSelectorDegressiveRatio());
        }
        ...
        for (Float targetRatio : sortedDesiredAspectRatios) {
            for (ResolutionBucket bucket : buckets) {
                ...
                if (Math.abs(aspectRatio.floatValue() - targetRatio) <= ASPECT_RATIO_TOLERANCE) {
                    result.addAll(pickUpToThree(bucket.sizes, isBackCamera));
                }
            }
        }
        return result;
     ...
     *         sizes from the list.
     */
    private static List<Size> pickUpToThree(List<Size> sizes, boolean isBackCamera) {
        List<Size> result = new ArrayList<Size>();
        Size largest = sizes.get(0);
        result.add(largest);
        Size lastSize = largest;

        int limitSize = CameraCustomInterpol.getLength(isBackCamera);
        int maxResultSize =FreemeCameraUtil.isPictureSizeCustomEnable() && limitSize != -1 ? limitSize : 3;
        if (CameraCustomManager.I.isOnlyDisplayPhotoProportion()){
            maxResultSize = 1;
        }

        for (Size size : sizes) {
            double targetArea = Math.pow(degressiveRatio, result.size()) * area(largest);
        ...
        if (result.size() < maxResultSize && !result.contains(lastSize)) {
            result.add(lastSize);
        }
        return result;
    }
