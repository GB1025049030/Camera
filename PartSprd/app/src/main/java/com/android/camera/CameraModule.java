...
public abstract class CameraModule extends DreamModule implements ModuleController {
    ...
    public boolean isPhoneCalling(Context context) {
        ...
        for (int i = 0; i < telephonyManager.getPhoneCount(); i++) {
            try{
                if (telephonyManager.getCallState(SubscriptionManager.getSubId(i)[0]) == TelephonyManager.CALL_STATE_OFFHOOK) {
                    isInCall = true;
                    break;
                }
  ...
  //SPRD:fix bug 613799 save the panorama exception
    ...
    public void resume() {
        CameraActivity activity = (CameraActivity) mAppController;
        activity.setEffectViewVisibility(false);
    }
