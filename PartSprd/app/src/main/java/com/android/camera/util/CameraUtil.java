...
import com.agenew.camera.FeatureOption;//add by hanqianqian for AGENEW_DEFAULT_FRONT_FLASH
import com.freeme.camera.ui.ModuleInfoResolve;
...
public class CameraUtil {
    ...
    public static final int SLOW_VIDEO_RECORD_SPEED_STANDARD = 1;
    public static final int SLOW_VIDEO_RECORD_SPEED_DEFAULT = 3;
    ...
    public static ModuleInfoResolve mModuleInfoResolve = new ModuleInfoResolve();
...
//    }
    ...
    public final static int BACK_MACRO_CAMERA_ID = 4;
    ...
    private static boolean isAgeShowEnabled = true;
    private static boolean isRaceShowEnabled = true;
    private static boolean isGenderShowEnabled = true;
    private final static String FACE_ATTRIBUTE_SHOW_ITEM = "persist.sys.cam.showfaceattribute";
    ...
    public static void initialize(Context context) {
        ...
        isColorEffectEnabled = isEnable(TARGET_COLOR_EFFECT_ENABLE, true);
        ...
        mLowBatteryNoFlashLevel = getLowBatteryLevel(context);
        ...
        /* SPRD: Add for bug 594960, beauty video recoding @{ */
        isMakeupVideoEnable = isEnable(TARGET_MAKE_UP_VIDEO, false);
        ...
        isNavigationEnable = "0".equals(android.os.SystemProperties.get("qemu.hw.mainkeys"));
        //SPRD:bug896943 Add config_showNavigationBar judgment.
        if (!isNavigationEnable){
            final Resources res = context.getResources();
            isNavigationEnable = res.getBoolean(com.android.internal.R.bool.config_showNavigationBar);
        }
        ...
        isQuickCaptureEnabled = isEnable(TARGET_TS_QUICK_CAPTURE_ENABL,true);
    ...
    public static int getLowBatteryLevel(Context context) {
        int lowBatteryLevel = android.os.SystemProperties.getInt(TARGER_LOW_BATTERY_NO_FLASH, 10);
        try {
            ...
            lowBatteryLevel = context.getResources().getInteger(field.getInt(null));
    ...
    private static void initFaceAttributeShowItem(){
        int showItem = getIntProperties(FACE_ATTRIBUTE_SHOW_ITEM,7);
        Log.i(TAG,"persist.sys.cam.showfaceattribute = " + showItem);
        isAgeShowEnabled = ((byte)showItem & 0x01) == 0x01;
        isGenderShowEnabled = ((byte)showItem & 0x02) == 0x02;
        isRaceShowEnabled = ((byte)showItem & 0x04) == 0x04;
    }
    ...
    public static boolean hasCutout() {
        try {
            ...
            Log.i(TAG, "NoSuchMethodError:", ex.getMessage());
            return true;
        }
        return true;
    }
    ...
    public  static void initStatusBarHeight(Context context) {
        ...
        if(resourceId > 0) {
            mStatusBarHeightPixel = res.getDimensionPixelSize(resourceId);
        }
        mStatusBarHeightPixel = 0;
    }
    ...
    public static boolean isHighResolutionSupport(){
    ...
    public static boolean isQrCodeEnabled(){
        return CameraCustomManager.I.isSupportQrCodeMode() && isQrCodeEnabled;
    }
    /* @} */
    ...
    public static boolean isModeBeautySupport(Context context){
        return false;
    };

    public static boolean isVidoWBEnabled() {
        return false;
    }
    ...
    public static boolean isContinuePhotoEnabled() {
        return CameraCustomManager.I.isSupportContinueMode() && isContinuePhotoEnabled && !isBack4in1Sensor();
    }
    public static boolean isManualPhotoEnable() {
        return CameraCustomManager.I.isSupportManualMode() && !isBackYUVSensor();
    }

    public static boolean isIntervalPhotoEnabled() {
        return CameraCustomManager.I.isSupportIntervalMode() && isIntervalPhotoEnabled;
    }
    ...
    public static boolean isZoomPanelEnabled(Context context) {
        int cameraId = DataModuleManager.getInstance(context).getDataModuleCamera().getInt(Keys.KEY_CAMERA_ID);
        if (DreamUtil.getRightCamera(cameraId) == DreamUtil.FRONT_CAMERA) {
            return CameraCustomManager.I.isSupportFrontZoomPanel() && isZoomPanelEnabled;
        } else {
            return CameraCustomManager.I.isSupportBackZoomPanel() && isZoomPanelEnabled;
        }
    }
   ...
   /*SPRD: add for burstmode album. @{ */
    ...
    /* @} */
    public static boolean isAgeShowEnabled() {
        return isAgeShowEnabled;
    }
    public static boolean isRaceShowEnabled() {
        return isRaceShowEnabled;
    }
    public static boolean isGenderShowEnabled() {
        return isGenderShowEnabled;
    }
    ...
    public static boolean isCameraBeautyEnable() {
        return !FreemeCameraUtil.isByteDanceEnabled() && mCurrentBeautyVersion != DISABLE_BEAUTY;
    }
...
//        saveBitmapToFile(bitmap);
    ...
    public static void saveYUVToFile(byte[] data, int width, int height) {
        SimpleDateFormat sDateFormat = new SimpleDateFormat("yyyy-MM-dd-hh-mm-ss-SSS");
        String date = sDateFormat.format(new java.util.Date());

        File d = new File("/storage/emulated/0/yuv/");
        File f = new File("/storage/emulated/0/yuv/", date + "-" + width + "x" + height + ".yuv");
    ...
    public static boolean isFlashSupported(CameraCapabilities capabilities, boolean isFront) {
        ...
        if(capabilities == null){
            ...
            return (mFrontFlashMode==VALUE_FRONT_FLASH_MODE_LCD) && FeatureOption.AGENEW_DEFAULT_OPEN_FRONT_FLASH;//add by hanqianqian for AGENEW_DEFAULT_OPEN_FRONT_FLASH
        } else {
    ...
    public static boolean isUseSprdFilter() {
        return CameraCustomManager.I.isSupportFilterMode() && mFilterVersion == 2;
    }
    ...
    public static boolean hasBlurRefocusCapture() {
        if (mBackBlurRefocusVersion != 0 && CameraCustomManager.I.isSupportSprdBackBlur()) {
            return true;
        } else {
            return false;
        }
    ...
    public static boolean hasFrontBlurRefocusCapture() {
        if (false && !isFrontYUVSensor() && mFrontBlurRefocusVersion != 0) {
            return true;
        } else {
            return false;
        }
    ...
    public static boolean isTcamAbility(Context mContext,int mModuleIndex, int mCameraId){
        return isTcamEnable() && ( SettingsScopeNamespaces.AUTO_PHOTO == mModuleIndex || SettingsScopeNamespaces.AUTO_VIDEO == mModuleIndex) && !DreamUtil.isFrontCamera(mContext,mCameraId);
    }
