...
import com.freeme.camera.common.gradienter.GradienterView;
...
import com.dream.camera.effect.EffectView;
...
import com.freeme.camera.AnimationShutterButton;
import com.freeme.camera.settings.FreemeSettingsScopeNamespaces;
import com.freeme.camera.ui.ModeScrollAdapter;
import com.freeme.camera.ui.ModeScrollView;
import com.freeme.camera.ui.StickyBottomModeScrollLayout;
...
public class CameraAppUI implements ModeListView.ModeSwitchListener,
    ...
    private FocusRingView mFocusRing;
...
//    private FrameLayout mTutorialsPlaceHolderWrapper;
    ...
    private SurfaceView mBottomCoverView;
    ...
    private final CameraModuleScreenShotProvider mCameraModuleScreenShotProvider
        ...
        @Override
        public Bitmap getScreenShot(Bitmap bmp, RectF previewArea) {
            previewArea.bottom = previewArea.bottom + 200;
    ...
    private class MyTouchListener implements View.OnTouchListener {
        ...
        public boolean onTouch(View v, MotionEvent event) {
            ...
            mController.getCurrentModule().onTouchEvent(event);
    ...
    public CameraAppUI(AppController controller,
        ...
        mGestureDetector = new GestureDetector(controller.getAndroidContext(), new MyGestureListener());
...
//        initBottomButtons();
    ...
    public void setSwipeEnabled(boolean enabled) {
        ...
        if (null != mModeScrollView) {
            mModeScrollView.setScrollable(enabled && initModeListViewEnd);
        }
        if (null != mModeTagShowLayout){
            mModeTagShowLayout.setClickable((View.VISIBLE == mModeTagShowLayout.getVisibility()) && enabled);
        }
    ...
    private void onSwipeDetected(int swipeState) {
        ...
        if (swipeState == SWIPE_UP || swipeState == SWIPE_DOWN) {
            ...
            if (currentModuleIndex != moduleToTransitionTo) {
                ...
                int iconRes = CameraUtil.mModuleInfoResolve.getModuleCoverIcon(moduleToTransitionTo);
            ...
            if (mModeScrollView == null) {
                return;
            }
            mModeScrollView.scrollRight();
    ...
    public void showSettingLayout() {
        initSettingLayout((SettingUIListener) getDreamInterface());
        ((CameraActivity)mController).updateDataModuleCamera();
        if (mSettingLayout != null)
            mSettingLayout.showSettingLayout();
    }
    ...
    public void resume() {
        ...
        if (initModeListViewEnd && null != mModeScrollView) {
            int indexInScroll = CameraUtil.mModuleInfoResolve.getModuleScrollAdapterIndex(
                    mController.getCurrentModuleIndex());
            int currentSelect = mModeScrollView.getCurrentIndex();
            if (currentSelect != indexInScroll) {
                changeBottomScrollView(mController.getCurrentModuleIndex(), false);
            }
        }
    ...
    public void showModeCoverUntilPreviewReady() {
        ...
        int iconId = CameraUtil.mModuleInfoResolve.getModuleCoverIcon(modeId);
    ...
    public boolean onBackPressed() { 
        if (mSettingLayout != null && mSettingLayout.getVisibility() == View.VISIBLE) {
            ...
            return handleModeListBackPress();
        }
        return false;
    }
...
//                true /* skipScopeCheck */);
    ...
    public void prepareModuleUI() {
        ...
        mEffectView = (EffectView) mCameraRootView.findViewById(R.id.preview_content_effectview);
        ...
        if((null != mEffectView)) {
            mEffectView.setCameraAppUI(this);
            mTextureViewHelper.addPreviewAreaSizeChangedListener(mEffectView.getPreviewAreaChangedListener());
        }
        /* @} */
        ...
        mTextureViewHelper.addPreviewAreaSizeChangedListener(mFocusRing);
                ...
                .addAspectRatioChangedListener(new PreviewStatusListener.PreviewAspectRatioChangedListener() {
                    ...
                    public void onPreviewAspectRatioChanged(float aspectRatio) {
                        ...
                        if (null != mSurfaceView) {
                            ...
                            mEffectView.setAspectRatio(aspectRatio);
                        }
                        /* @} */
                    }
                });
        mGLPreviewRoot = mCameraRootView.findViewById(R.id.gl_preview_parent);
    }
...
//        mTutorialsPlaceHolderWrapper.setVisibility(View.GONE);
    ...
    public void onPreviewStarting() {
        mController.setupPreviewDataListener();
    }
    ...
    @Override
    public void onCameraIdUpdate(int modeIndex) {
        //mController.onModeSelected(modeIndex);
        //camera_support_all -1
        //camera_support_back 0
        //camera_support_front 1
        //camera_support_none 999
        int currentCamera = DreamUtil.getRightCamera(mDataModule.getIndexOfCurrentValue(Keys.KEY_CAMERA_ID));
        //next mode camera support.
        int cameraSupport = CameraUtil.mModuleInfoResolve.getModuleSupportCamera(modeIndex);
        boolean needToggleCamera = (mController.getAndroidContext().getResources().getInteger(R.integer.camera_support_all) != cameraSupport)
                && (currentCamera != cameraSupport);
        int nextCamera = currentCamera;
        if (needToggleCamera) nextCamera = currentCamera > 0 ? currentCamera - 1 : 1;
        Log.i(TAG, "onModeScrolled needToggleCamera = " + needToggleCamera);
        mDataModule.set(Keys.KEY_CAMERA_ID, nextCamera);
    }
    ...
    public EffectView getmEffectView() { return mEffectView;}
    ...
    public void setBottomBarShutterIcon(int modeIndex) {
        ...
        int shutterIconId = CameraUtil.mModuleInfoResolve.getModuleCaptureIcon(modeIndex);
    ...
    public void setIndicatorBottomBarWrapperVisible(boolean visible) {
        ...
        if (mStickyBottomModeScrollLayout != null) {
            mStickyBottomModeScrollLayout.setVisibility(visible && showPanel ? View.VISIBLE : View.INVISIBLE);
        }
    }
...
//    }
    ...
    private boolean handleModeListBackPress() {
        if (!mModeListView.isShown()) return false;
        boolean handle = mModeListView.onBackPressed();
        if(!mController.isSecureCamera() && handle && initModeScrollEnd && null != mModeScrollView){
            mModeScrollView.setCurrentIndex(
                    CameraUtil.mModuleInfoResolve.getModuleScrollAdapterIndex(mController.getCurrentModuleIndex()), true);
        }
        return handle;
    }
    ...
    public void updateSwitchModeBtnVisibility() {
        if (switchModeBtn == null) {
            switchModeBtn = mCameraRootView.findViewById(R.id.btn_mode_switch);
        }
        if (switchModeBtn != null) {
            switchModeBtn.setVisibility(getSwitchModeBtnVisibility(View.VISIBLE));
        }
    }

    private int getSwitchModeBtnVisibility(int visibility) {
        int currentMode = mController.getModeIndex();
        int mcCamera = CameraUtil.mModuleInfoResolve.getModuleSupportCamera(currentMode);
        return (mcCamera != -1 ? View.INVISIBLE : visibility);
    }
    ...
    public void changeToRecordingUI() {
        changeToRecordingUI(false);
    }

    public void changeToRecordingUI(boolean isForceGone) {
        ...
        if (isForceGone || !mController.getCurrentModule().isSupportVideoCapture()) {
            videoCaptureBtn.setVisibility(View.GONE);
    ...
    public void setSlowMotionCaptureButtonDisable() {
        setSlowMotionCaptureButtonDisable(false);
    }

    public void setSlowMotionCaptureButtonDisable(boolean isForceGone) {
        boolean isSupportVideoCapture = mController.getCurrentModule().isSupportVideoCapture();
        videoCaptureBtn.setVisibility(
                isForceGone || !isSupportVideoCapture ? View.GONE : View.VISIBLE);
        videoCaptureBtn.setEnabled(false);
    }
 ...
 */
   private boolean invalidModule(int moduleId) {
    ...
    public void setBottomBarLeftAndRightUI(int visible) {
        ...
        if (switchModeBtn != null) {
            switchModeBtn.setVisibility(getSwitchModeBtnVisibility(visible));
        }
...
//    }
    ...
    public Bitmap getPreviewOverlayAndControls() {
        boolean showModeScroll = false;
        if (null != mModeScrollView) {
            if (mModeScrollView.getVisibility() == View.VISIBLE) {
                mModeScrollView.setVisibility(View.INVISIBLE);
                showModeScroll = true;
            }
        }
        if (null != mModeScrollView && showModeScroll) mModeScrollView.setVisibility(View.VISIBLE);
    ...
    public void hideBottomPanelLeft() {
        if (bottomPanelLeft != null)
            bottomPanelLeft.setVisibility(View.INVISIBLE);
        updateAiSceneView(View.INVISIBLE , 0);
    }
    ...
    public void setEffectHolderListener(SurfaceHolder.Callback surfaceHolderListener) {
        if(mEffectView != null) {
            mEffectView.setSurfaceHolderListener(surfaceHolderListener);
        }
    }

    public void setEffectViewVisibility(boolean visible) {
        if(mEffectView != null) {
            mEffectView.setVisibility(visible? View.VISIBLE : View.GONE);
        }
    }
    ...
    private ViewGroup sidePanelParent;
    public void initSidePanel(){
        if (sidePanel == null) {
            sidePanel = mCameraRootView.findViewById(R.id.side_panel_parent);
            sidePanelParent = (ViewGroup) sidePanel;
        }
    ...
    public ViewGroup getSidePanelParent(){
        return sidePanelParent;
    }
    ...
    public void initBottomBar() {
        ...
        if (mStickyBottomCaptureLayout == null) {
            ...
            mStickyBottomModeScrollLayout = (StickyBottomModeScrollLayout) mAppRootView.findViewById(R.id.sticky_bottom_modescroll_layout);
            mStickyBottomModeScrollLayout.setCaptureLayoutHelper(mCaptureLayoutHelper);
        }
        // if (mRoundedThumbnailView == null){
    ...
    public void updateBottomBar(){
        ...
        resetBottomControls(mController.getCurrentModuleController(), mController.getCurrentModuleIndex());
        changeBottomScrollView(mController.getCurrentModuleIndex(), true);
    }

    public void updateBottomFrame(RectF bottomBarRect, int marginBottom) {
        FrameLayout bottomFrame = mCameraRootView.findViewById(R.id.bottom_frame);
        FrameLayout.LayoutParams layoutParams = (FrameLayout.LayoutParams) bottomFrame.getLayoutParams();
        layoutParams.setMargins(0, 0, 0, marginBottom);
        layoutParams.height = (int) bottomBarRect.height();
        bottomFrame.setLayoutParams(layoutParams);
    }

    public void updateExtendLayoutParams(ViewGroup extendPanelParent, int uiType) {
        LinearLayout.LayoutParams extendParentParams = (LinearLayout.LayoutParams) extendPanelParent.getLayoutParams();
        extendPanelParent.setLayoutParams(extendParentParams);
    }

    public SurfaceView getBottomCoverView(){
        if (mBottomCoverView == null){
            mBottomCoverView = mAppRootView.findViewById(R.id.bottom_cover);
        }
        return mBottomCoverView;
    }
    ...
    public void initGradienterView() {
        ViewStub viewStubGradienterView = mCameraRootView.findViewById(R.id.layout_gradienter_view_id);
        if (viewStubGradienterView != null) {
            viewStubGradienterView.inflate();
        }
    }

    public void updateGradienterView(boolean isShow) {
        if (mGradienterView == null) {
            mGradienterView = mCameraRootView.findViewById(R.id.gradienter);
        }
        if (mGradienterView != null) {
            mGradienterView.setVisibility(isShow ? View.VISIBLE : View.GONE);
            updateGradienterState(isShow);
        }
    }

    public void updateGradienterState(boolean state) {
        if (mGradienterView != null) {
            if (state)
                mGradienterView.start();
            else
                mGradienterView.stop();
        }
    }
    ...
    public void setBlurEffectHighlight(boolean highlight) {
        ...
        //*/agn add by wucheng 20200701 begin
        if (mBlurEffectTip.getVisibility() == View.GONE && !com.agenew.camera.FeatureOption.AGENEW_CAMERA_BV) {
    ...
    public boolean isModeTransitionViewShow() {
        if (View.VISIBLE == mModeTransitionView.getVisibility()) {
            return true;
        }
        return false;
    }
    ...
    public boolean unResponseClick() {
        ...
        if (!mController.getCurrentModule().isEnableDetector()) {
    ...
    private boolean initModeScrollEnd = false;
    private ModeScrollView mModeScrollView;
    private LinearLayout mModeTagShowLayout;
    private StickyBottomModeScrollLayout mStickyBottomModeScrollLayout;
    private void initHorizontalModeScroll() {
        ...
        if (secureCam) {
            mModuleNameList.add(CameraUtil.mModuleInfoResolve.getModuleText((SettingsScopeNamespaces.AUTO_PHOTO)));
            mModuleNameList.add(CameraUtil.mModuleInfoResolve.getModuleText((SettingsScopeNamespaces.AUTO_VIDEO)));
        } else {
            ...
            for (int i = 0; i < panelSize; i++) {
                String name = CameraUtil.mModuleInfoResolve.getModuleText(panelShowList.get(i));
                mModuleNameList.add(name);
            }
        }
        ...
        mModeScrollView.setAdapter(hAdapter);
        mModeScrollView.setOnSelectChangeListener(new ModeScrollView.OnSelectChangeListener() {
            @Override
            public void onSelectChange(int lastposition, int position, int smoothToPosition) {
                if (mModeListView != null && mModeListView.isShown()) {
                    mModeListView.onBackPressed();
                }
                ArrayList<Integer> panelShowList = CameraUtil.mModuleInfoResolve.getModeSlideList();
                if (position >= panelShowList.size()) return;
                int nextSelectId = secureCam
                        ? (position == 0 ? SettingsScopeNamespaces.AUTO_PHOTO : SettingsScopeNamespaces.AUTO_VIDEO)
                        : panelShowList.get(position);
                Log.d(TAG, "setOnSelectChangeListener onSelectChange index=" + nextSelectId + ", getCurrentModuleIndex=" + mController.getCurrentModuleIndex());
                if (nextSelectId == mController.getCurrentModuleIndex()) {
                    onModeScrollFreezeShow(smoothToPosition);
                    return;
                }
                if (!onModeScrolled(nextSelectId, smoothToPosition)){
                    //if scroll failed, reset the scroll item state.
                    mModeScrollView.setCurrentIndex(lastposition,false);
                }
            }
        });
        mModeScrollView.setCurrentIndex(secureCam ? 0 : CameraUtil.mModuleInfoResolve.getModuleScrollAdapterIndex(mController.getCurrentModuleIndex()), false);
        initModeScrollEnd = true;
    }


    public void onModeScrollFreezeShow(int smoothToPosition){
        //notify modescrollview can start smoothscroll anim.
        mModeScrollView.smoothScrollAfterFreeze(smoothToPosition);
    }


    public boolean onModeScrolled(int modeIndex, int smoothToPosition) {
        if (999 == modeIndex) {
            if (!showSettingsUI(SWIPE_RIGHT) && (mSettingLayout == null
                    || !mSettingLayout.isShown()) && mModeListView != null && mModeListView.isInitDone()) {
                mModeListView.setShouldBeVisible(true);
                mModeListView.setVisibility(View.VISIBLE);
                mController.getMainHandler().postDelayed(() -> mAppRootView.redirectTouchEventsTo(mModeListView), 100);
                onModeScrollFreezeShow(smoothToPosition);
                return true;
            } else {
                return false;
        ...
        onCameraIdUpdate(modeIndex);
        //mModeScrollView.setVisibility(View.INVISIBLE);
        //mode switch with freezeScreen.
        mController.dealModeFreeze(modeIndex, smoothToPosition, true);
        return true;
    }

    public void changeBottomScrollView(int modeId, boolean updateModeTag) {
        if (mModeScrollView == null) return;
        if (initModeListViewEnd && mModeListView.isModeInSelector(modeId)) {
            if (!updateModeTag) return;
            mModeScrollView.setVisibility(View.INVISIBLE);
            mModeScrollView.setScrollable(false);
            mModeTagShowLayout.setVisibility(View.VISIBLE);
            TextView modeSingleTextView = (TextView) mModeTagShowLayout.findViewById(R.id.bottom_modeid_show_text);
            modeSingleTextView.setText(CameraUtil.mModuleInfoResolve.getModuleText(modeId));
            mModeTagShowLayout.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View arg0) {
                    if (mController.getCurrentModuleIndex() == FreemeSettingsScopeNamespaces.IKO_PHOTO) {
                        mController.checkPrivacyState();
                    }
                    mController.dealModeFreeze(mController.getModeIndexDefault(), 0, false);
                }
            });
        } else {
            mModeScrollView.setVisibility(View.VISIBLE);
            mModeScrollView.setScrollable(mSwipeEnabled);
            boolean isSecure = mController.isSecureCamera();
            if (!isSecure && mController.getModeIndexDefault() == modeId) {
                mModeScrollView.setCurrentIndex(CameraUtil.mModuleInfoResolve.getModuleScrollAdapterIndex(mController.getModeIndexDefault()), false);
            } else if (!isSecure){
                mModeScrollView.setCurrentIndex(CameraUtil.mModuleInfoResolve.getModuleScrollAdapterIndex(modeId), false);
            } else if (isSecure) {
                mModeScrollView.setCurrentIndex(modeId == SettingsScopeNamespaces.AUTO_VIDEO ? 1 : 0, false);
            }
            mModeTagShowLayout.setVisibility(View.INVISIBLE);
        }
    }


    public void setModeScrollVisible(int visible) {
        if (mModeScrollView == null) return;
        mModeScrollView.setVisibility(visible);
    }


    public void setModeScrollPanelShow(boolean show) {
        if (mStickyBottomModeScrollLayout == null) return;
        mStickyBottomModeScrollLayout.setVisibility(show ? View.VISIBLE : View.INVISIBLE);
    }


    public void hideModeListInstant() {
        if (mModeListView != null && mModeListView.isShown()) {
            mModeListView.hide();
        }
    }
    ...
    public ModeScrollView getModeScrollView(){
        return mModeScrollView;
    }
    ...
    protected void inflateLayout() {
        ...
        ViewStub viewStubBottomScroll = (ViewStub) mAppRootView
                .findViewById(R.id.dream_sticky_bottom_modescroll_id);
        if (viewStubBottomScroll != null) {
            viewStubBottomScroll.inflate();
        }
    ...
    public void setCenterScanText(String str) {
        mBottomBar.setCenterScanText(str);
    }

    public void setShutterButtonNeedAnimation(boolean isNeed) {
        if (mShutterButton == null) return;
        AnimationShutterButton button = (AnimationShutterButton) mShutterButton;
        if (button.getNeedAnimation() != isNeed) {
            button.setNeedAnimation(isNeed);
        }
    }

    public DreamCaptureLayoutHelper getCaptureLayoutHelper() {
        return mCaptureLayoutHelper;
    }

    private FrameLayout mGLPreviewRoot;
    public ViewGroup getGLPreviewRoot() {
        return mGLPreviewRoot;
    }
}
