...
public abstract class DreamUISettingPartBasic extends PreferenceCategory
    ...
    public boolean getPersistedBoolean(String key) {
        if (mDataModule != null) {
            if (getPersistedSpecialBoolean(key)) {
                return false;
            }
    ...
    public boolean persistBoolean(String key, boolean value) {
        if (mDataModule != null) {
            if (persistSpecialBoolean(key, value)) return true;
...
//        mDataModule.changeSettings(preference.getKey(), newValue);
    ...
    private boolean getPersistedSpecialBoolean(String key) {
        if (mDataModule != null) {
            if (Keys.KEY_RECORD_LOCATION.equals(key) || FreemeKeys.KEY_LOCATION_WATER_MARK.equals(key)) {
                if (mDataModule.getBoolean(key)) {
                    CameraActivity cameraActivity = (CameraActivity) getContext();
                    if (!cameraActivity.isPrivacyEnable()) {
                        mDataModule.changeSettings(key, false);
                        return true;
                    }
                }
            }
    ...
    private boolean persistSpecialBoolean(String key, boolean value) {
        if (mDataModule != null) {
            if (value && (Keys.KEY_RECORD_LOCATION.equals(key)
                    || FreemeKeys.KEY_LOCATION_WATER_MARK.equals(key))) {
                CameraActivity cameraActivity = (CameraActivity) getContext();
                if (cameraActivity.isPrivacyEnable() && cameraActivity.checkLocationPermissions()) {
                    mDataModule.changeSettings(key, true);
                } else {
                    mDataModule.changeSettings(key, false);
                    cameraActivity.checkPrivacyAndLocationPermission(CameraActivity.getLocationRequestCode(key));
                }
                return true;
            }
        }
