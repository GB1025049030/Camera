...
public class DreamUtil {
    ...
    public String getGlobleKey(int cameraId) {
        String key = Keys.KEY_BACK_MODE;
        switch (cameraId) {
            case BACK_CAMERA:
                key = Keys.KEY_BACK_MODE;
                break;
            case FRONT_CAMERA:
                key = Keys.KEY_FRONT_MODE;
                break;
        }
        return key;
    }
    ...
    public int getDefaultValue(int cameraId) {
    ...
    public int getRightMode(DataModuleBasic dataModule, int cameraId) {
        Log.d(TAG,
                "getRightMode cameraId=" + cameraId);
        return dataModule.getInt(
                getGlobleKey(cameraId), getDefaultValue(cameraId));
    }
...
//                R.array.dream_module_mode_support_list);
        ...
        if (saveMode == SettingsScopeNamespaces.FRONT_BLUR) {
            ...
            dataModule.set(Keys.KEY_FRONT_MODE, saveMode);
            return;
        } else if (saveMode == SettingsScopeNamespaces.REFOCUS) {
            //dataModule.set(Keys.KEY_BACK_PHOTO_MODE, saveMode);
            dataModule.set(Keys.KEY_BACK_MODE, saveMode);
            return;
        }
        //dataModule.set(getGlobleKey(modeSupportList[saveMode], cameraId), saveMode);
        //dataModule.set(getGlobleKey(CameraUtil.mModuleInfoResolve.getModuleSupportMode(saveMode), cameraId), saveMode);
        //dataModule.set(getGlobleKey(cameraId), saveMode);
        saveToCameraMode(dataModule, cameraId, saveMode);
    }

    private void saveToCameraMode(DataModuleBasic dataModule, int cameraId, int saveMode) {
        int mcCamera = CameraUtil.mModuleInfoResolve.getModuleSupportCamera(saveMode);
        if (mcCamera == -1) {
            dataModule.set(getGlobleKey(DreamUtil.FRONT_CAMERA), saveMode);
            dataModule.set(getGlobleKey(DreamUtil.BACK_CAMERA), saveMode);
        } else {
            dataModule.set(getGlobleKey(cameraId), saveMode);
        }
    }
