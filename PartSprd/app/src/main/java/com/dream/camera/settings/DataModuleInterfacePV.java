...
import com.freeme.camera.common.custom.CameraCustomXmlParser;
...
public abstract class DataModuleInterfacePV extends DataModuleBasic {
    ...
    private static DecimalFormat sMegaPixelFormat = new DecimalFormat("##0");
    ...
    protected String getSizeSummaryString(Size size) {
        ...
        if (CameraCustomManager.I.isOnlyDisplayPhotoProportion()){
            result = mContext.getResources().getString(
                    R.string.setting_summary_photo_proportion,
                    numerator, denominator);
            if (numerator == 4 && denominator == 3){
                result = result + mContext.getResources().getString(R.string.camera_picture_proportion_recommend);
            }
        }else {
            result = mContext.getResources().getString(
    ...
    protected String getSizeSummaryString(Size size, int id, String key) {
        String result = null;
        if (FreemeCameraUtil.isPictureSizeCustomEnable()) {
            String sizeKey = String.valueOf((Keys.KEY_PICTURE_SIZE_FRONT.equals(key) ? 8 : 0) | id);
            result = CameraCustomManager.I.getPictureSize(sizeKey);
        }
        if (TextUtils.isEmpty(result)) {
            result = getSizeSummaryString(size);
        }
        return result;
    }
    ...
    protected void setEVSVideoQualities(String key,
        ...
        if (key.equals(Keys.KEY_VIDEO_QUALITY_BACK) || key.equals(Keys.KEY_VIDEO_QUALITY_FRONT)) {
        ...
        if (null != (DataStorageStruct) mSupportDataMap
            ...
            || null != (DataStorageStruct) mSupportDataMap
                .get(Keys.KEY_SLOW_VIDEO_QUALITY_BACK)
            || null != (DataStorageStruct) mSupportDataMap
                .get(Keys.KEY_SLOW_VIDEO_QUALITY_FRONT)
            || null != (DataStorageStruct) mSupportDataMap
                .get(Keys.KEY_EFFECT_VIDEO_QUALITY_BACK)
            || null != (DataStorageStruct) mSupportDataMap
                .get(Keys.KEY_EFFECT_VIDEO_QUALITY_FRONT)) {
    ...
    protected void setEVSPicturesize(String key, List<Size> selectedSizes) {
        ...
        for (int i = 0; i < selectedSizes.size(); i++) {
            ...
            entries[i] = getSizeSummaryString(size, i, key);
        ...
        data.mDefaultValue = data.mEntryValues[0].toString();
        /*if (key == Keys.KEY_PICTURE_SIZE_BACK) {
            if (data.mEntryValues.length > 1) {
