...
abstract public class FilterModuleAbs extends PhotoModule implements
    ...
    public void init(CameraActivity activity, boolean isSecureCamera,
        ...
        int backCameraId = activity.getCameraProvider().getFirstBackCameraId();
        int frontCameraId = activity.getCameraProvider().getFirstFrontCameraId();

        if((backCameraId != NO_DEVICE_AVAILABLE) && (frontCameraId != NO_DEVICE_AVAILABLE)){
            ...
            FilterTranslationUtils.getInstance().setSensorOrientation(
                    infoF.getSensorOrientation(), infoB.getSensorOrientation());
        }else{
            if((backCameraId != NO_DEVICE_AVAILABLE) && (frontCameraId == NO_DEVICE_AVAILABLE)){
                Characteristics infoB = activity.getCameraProvider()
                        .getCharacteristics(Camera.CameraInfo.CAMERA_FACING_BACK);
            ...
            if((backCameraId == NO_DEVICE_AVAILABLE) && (frontCameraId != NO_DEVICE_AVAILABLE)){
                ...
                FilterTranslationUtils.getInstance().setSensorOrientation(
                        infoF.getSensorOrientation(), 0);

            }
        }

        /*
         * SPRD:Add for bug 458160 filter mode's resolution is not the same as
         * photo mode's @{
         */
        // Toast.makeText(mActivity, R.string.filter_warning,
        // Toast.LENGTH_LONG).show();//SPRD:fix bug605019
        /* @} */
    }
    ...
    protected void updateFilterDisplay(int state) {
        super.updateFilterDisplay(state);
        if (state == 0) {
            ((FilterModuleUIAbs) mUI).onPanelsHidden();
        } else {
            ((FilterModuleUIAbs) mUI).onPanelsShown();
        }
    }
}
