...
public class AutoPhotoUI extends PhotoUI implements ResetListener {
    ...
    protected int getSidePanelConfigID() {
        return R.array.auto_photo_side_panel;
    }
    ...
    public void showBeautyButton(boolean show) {
        if (show) {
            ...
            if(mPanelManager.isContain(R.integer.make_up_display_toggle_button_dream)){
                mPanelManager.showButton(R.integer.make_up_display_toggle_button_dream);
            }
        } else {
            Log.i(TAG , "make Beauty Button GONE");
            if(mPanelManager.isContain(R.integer.make_up_display_toggle_button_dream)){
                mPanelManager.hideButton(R.integer.make_up_display_toggle_button_dream);
            }
        }
        if (mMakeupController != null) {
                if (show) {
                    ...
                    if (isBeauty == 1) // show makeup controller
                        mMakeupController.resume(DataModuleManager.getInstance(mActivity.getAndroidContext()).getCurrentDataModule().getBoolean(Keys.KEY_CAMERA_BEAUTY_ENTERED));
                    else
    ...
    public void fitExtendPanel(ViewGroup extendPanelParent) {
        if(mBasicModule.isBeautyCanBeUsed()) {
            ...
            lf.inflate(R.layout.sprd_autophoto_without_makeup_extend_panel, extendPanelParent);
        }
