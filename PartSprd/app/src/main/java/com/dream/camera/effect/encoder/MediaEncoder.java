...
//        	throw new NullPointerException("muxer is unexpectedly null");
        ...
        while (mIsCapturing) {
            ...
            if (encoderStatus == MediaCodec.INFO_TRY_AGAIN_LATER) {
                ...
                if (mimeType.startsWith("audio")) {
                    mAudioTrack = mTrackIndex;
                } else if(mimeType.startsWith("video")) {
                    mVideoTrack =  mTrackIndex;
                }
                mMuxerStarted = true;
                if (!muxer.start()) {
    ...
    protected long getPTSUs() {
        ...
        long time;
        if(mSpeed > 1) {
            if (mTrackIndex == mVideoTrack) {
                time = (result - baseTimeStamp - pauseDelayTime) / 1000 * mSpeed;
            } else {
                time = (result - baseTimeStamp - pauseDelayTime) / 1000;
            }
        } else {
            time = (result - baseTimeStamp - pauseDelayTime) / 1000;
        }
