...
public class SlrPhotoModule extends PhotoModule {
    private static final Log.Tag TAG = new Log.Tag("SlrPhotoModule");

    public SlrPhotoModule(AppController app) {
        super(app);
    }
    ...
    public PhotoUI createUI(CameraActivity activity) {
        ...
        return new SlrPhotoUI(activity, this, activity.getModuleLayoutRoot());
    }
...
//        startMotionRecord();
    ...
    private final class JpegPictureCallback extends PhotoModule.JpegPictureCallback {
        public JpegPictureCallback(Location loc) {
            super(loc);
        }

        @Override
        public byte[] getJpegData(byte[] jpegData) {
            return ((SlrPhotoUI) mUI).getBVirtualView().blendJpegData(jpegData);
        }
    }

    @Override
    public void onPreviewDataUpdate(byte[] data, CameraAgent.CameraProxy camera) {
        if (mCameraSettings != null && mCameraSettings.getCurrentPreviewSize() != null) {
            ((SlrPhotoUI) mUI).getBVirtualView().setPreviewData(data,
                    mCameraSettings.getCurrentPreviewSize().width(), mCameraSettings.getCurrentPreviewSize().height());
        }
    }

    @Override
    public boolean onTouchEvent(MotionEvent event) {
        ((SlrPhotoUI)mUI).getBVirtualView().onTouchEvent(event);
